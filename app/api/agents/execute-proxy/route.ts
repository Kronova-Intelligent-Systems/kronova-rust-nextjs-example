...
export async function POST(request: NextRequest) {
  try {
    ...
    const { agentId, prompt, assetIds, dataStreamIds, webhookUrl, apiKey: clientApiKey } = body

    if (!agentId || !prompt) {
      return NextResponse.json(
        { error: "Missing required fields: agentId and prompt are required" },
        {
          status: 400,
          headers: {
            "Content-Type": "application/json",
    ...
    if (session) {
      ...
      const { data: validationResult, error: validationError } = await supabase.rpc("validate_api_key", {
        ...
        x_mark_usage: true,
        x_required_scopes: JSON.stringify(["agents:execute"]),
      })
    ...
    if (!response.ok) {
      ...
      console.error("[v0] Agent execution failed:", errorText)
      return NextResponse.json(
        { error: "Agent execution failed", details: errorText },
        {
    ...
    console.error("[v0] Error in execute proxy:", error)
    return NextResponse.json(
      {
