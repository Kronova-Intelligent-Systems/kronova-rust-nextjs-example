-- Migration v2: Create advanced validate_api_key function with hash-based validation
-- Enterprise-grade security: Uses private schema with public wrapper pattern
-- This version ensures only ONE function signature exists to prevent ambiguity
-- Removes backward-compatible simple version, retaining only advanced version

-- Ensure pgcrypto extension is enabled
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- Drop ALL existing functions in both schemas with all possible signatures
-- This ensures we start completely clean and prevent function ambiguity
...
-- All variables prefixed with v_ to avoid ambiguous column references
...
BEGIN
  -- Query with explicit table alias (ak.) to avoid ambiguous column references
  SELECT 
    ak.id,
    ak.user_id,
    ak.scopes,
...
-- This is the ONLY function exposed to the REST API
...
'Advanced API key validation with hash verification, user validation, scope checking, and usage tracking. Returns validation result with detailed error messages. This is the ONLY exposed version - no backward-compatible simple version exists.';

COMMENT ON FUNCTION private.validate_api_key(text, text, uuid, boolean, jsonb) IS 
'Private implementation of advanced API key validation. Uses v_ prefix for all variables to avoid ambiguous column references. Validates key hash, prefix, user ownership, and required scopes. Accessed only through public wrapper.';
